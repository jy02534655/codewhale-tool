...
| `default_text_model` | 默认文本模型 | 因提供商而异 | 如 `deepseek-v4-pro`、`deepseek-v4-flash`、`claude-sonnet-4-6` 等。DeepSeek 官方当前推荐 `deepseek-v4-pro`（1M 上下文，384K 输出，默认启用 thinking）和 `deepseek-v4-flash`（快速路径） |
...
| `permissions.toml` | 同级权限规则文件 | `[[rules]]` 条目 | 支持 `tool` + 可选 `command` / `path` 字段。匹配的规则会在非 YOLO 模式下强制触发审批；`never` 模式下匹配则直接拒绝（因为无法弹窗） |
...
| 配置项 | 作用 | 默认值 / 范围 | 说明 |
|--------|------|---------------|------|
| `max_subagents` | 最大并发子代理数 | 默认 `20`，限制 `1~20` | 同时处于活跃状态的子代理数量上限 |
| `[subagents].max_concurrent` | 同上（优先级更高） | 默认 `20`，限制 `1~20` | 与 `max_subagents` 等价，但配置中优先级更高 |
| `[subagents].max_admitted` | 排队+运行总数上限 | 默认 `200`，范围 `max_concurrent`~`200` | 包括正在运行和排队等待的子代理总数。高扇出任务可以排队，但运行时启动压力受控 |
...
| `[subagents].api_timeout_secs` | API 超时 | 默认 `120`，范围 `1~1800` | 子代理单步模型调用的超时时间 |
...
| `[context].verbatim_window_turns` | 完整保留的最近轮数 | `16` | 最近多少轮对话保持完整原文，更早的轮次会被压缩或摘要 |
| `[context].l1_threshold` | L1 阈值 | `192000` | 第一级上下文压力阈值（Token 数）。超过此值触发轻量级干预 |
| `[context].l2_threshold` | L2 阈值 | `384000` | 第二级阈值。超过后采取更激进的压缩策略 |
| `[context].l3_threshold` | L3 阈值 | `576000` | 第三级阈值。接近模型上限时的紧急处理线 |
| `[context].seam_model` | 接缝模型 | `deepseek-v4-flash` | 执行上下文协调任务的模型。Flash 路径无 reasoning，速度更快 |
...
| | | `osc9` | 发送 OSC 9 转义序列 `\x1b]9;<msg>\x07`。在 tmux 中会自动包装 DCS 透传。支持 iTerm2、Terminal.app 13+、Ghostty、Kitty、WezTerm 等 |
| | | `bel` | 发送单个 `\x07`（BEL）字符。最保守的兼容性方案，几乎所有终端都支持响铃 |
| | | `off` | 完全关闭完成通知 |
...
| | | `auto` | 边栏在空闲时自动折叠，有内容时展开。保存此设置会记录显式的 auto-collapse 偏好 |
| | | `tasks` | 默认聚焦 Tasks 面板 |
| | | `agents` | 默认聚焦 Agents 面板 |
| | | `context` | 默认聚焦 Context 面板 |
| | | `hidden` | 完全隐藏右侧边栏，防止终端文本选择跨越到边栏边框 |
| `show_thinking` | 显示思考过程 | `on` / `off` | 是否在 TUI 对话记录中展示模型的 reasoning/thinking 内容 |
| `show_tool_details` | 显示工具详情 | `on` / `off` | 是否展开显示每次工具调用的详细参数和结果 |
| `auto_compact` | 自动压缩上下文 | `on` / `off` | 模型感知默认开启。当上下文接近模型窗口上限时，自动生成摘要并替换旧内容，延续到下一轮 |
| `auto_compact_threshold_percent` | 自动压缩阈值 | `10~100` | 默认 `80`。当活跃请求输入估算达到模型上下文窗口的该百分比时触发自动压缩 |
| `paste_burst_detection` | 粘贴突发检测 | `on` / `off` | 默认 `on`。检测不支持 bracketed-paste 事件的终端中的快速批量粘贴，防止逐字符触发补全 |
| `mention_menu_limit` | @提及菜单候选数 | 整数 | 默认 `128`。`@` 弹窗最多保留多少候选文件，超出部分截断 |
| `mention_walk_depth` | @提及遍历深度 | 整数 | 默认 `6`。`@` 补全遍历工作区的最大目录深度。设为 `0` 表示无限深度（适合深层嵌套仓库） |
| `mention_menu_behavior` | @提及菜单行为 | `fuzzy` | **默认**。模糊搜索整个工作区，并结合提及频率（frecency）排序 |
| | | `browser` | 仅列出当前已输入目录段的直接子目录，按字母顺序排列。更可控、确定性更强 |
| `cost_currency` | 成本显示货币 | `usd` | **默认**。美元 |
| | | `cny` | 人民币（别名 `rmb`、`yuan` 会归一化为 `cny`） |
| `background_color` | 自定义背景色 | `#RRGGBB` / `default` | 自定义 TUI 根、头部、对话记录、底部的背景色，同时保留面板对比度。`default` 使用主题默认 |
| `max_history` | 输入历史条数 | 整数 | 保留多少条已提交输入历史。已清除的草稿也保留在本地用于历史搜索 |
| `default_model` | 默认模型覆盖 | 字符串 | 覆盖当前提供商的默认模型选择 |
| `tui.alternate_screen` | 备用屏幕 | `auto` / `always` / `never` | 保留兼容性配置。实际上交互会话始终使用 TUI 拥有的备用屏幕，防止主机终端滚动条抢占视口 |
| `tui.mouse_capture` | 鼠标捕获 | `true` / `false` | 默认 `true`（非 Windows 终端和 Windows Terminal/ConEmu/Cmder 启用；JetBrains 终端默认关闭，因为鼠标事件会泄漏为乱码）。启用后支持内部滚动、对话选择、右键菜单、滚动条拖拽 |
| `tui.terminal_probe_timeout_ms` | 终端探测超时 | 整数 | 默认 `500`，范围 `100~5000`。启动时探测终端模式（如颜色支持、鼠标协议）的超时。超时则警告并终止启动，防止无限挂起 |
| `tui.stream_chunk_timeout_secs` | 流式块超时 | 整数 | 默认 `300`，范围 `1~3600`。SSE 流中两个数据块之间的最大空闲时间。慢速本地服务器可适当提高。`0` 映射为默认值 |
| `tui.osc8_links` | OSC 8 超链接 | `true` / `false` | macOS/Linux 默认 `true`，Windows 默认 `false`。在对话输出中的 URL 周围包裹 OSC 8 转义序列，支持 iTerm2、Ghostty、Kitty、WezTerm 等终端的 Cmd+点击跳转。不支持 OSC 8 的终端会忽略转义，只显示纯 URL |
| `verbosity` | 输出详细程度 | `normal` | **默认**。标准对话风格，包含解释和上下文 |
| | | `concise` | 简洁模式，追加提示纪律块要求直接、低冗余输出。CLI 非交互命令（`exec`、`eval`）默认使用此模式 |
...
| `[features].web_search` | 网页搜索 | `true` | 启用 `web_search` 和兼容别名 `web.run`，默认使用 DuckDuckGo |
...
| 配置项 | 作用 | 可选值 | 各值说明 |
|--------|------|--------|----------|
| `[search].provider` | 搜索后端 | `duckduckgo` | **默认**。无需 API Key。DuckDuckGo HTML 搜索，遇到机器人挑战时自动回退到 Bing |
| | | `bing` | 微软 Bing 搜索，可作为 DuckDuckGo 的回退或显式选择 |
...
| | | `baidu` | 百度 AI 搜索 `https://qianfan.baidubce.com/v2/ai_search/web_search`，需要 API Key |
...
| `[search].api_key` | API 密钥 | 字符串 | Tavily、Bocha、Baidu、Volcengine、Sofya 必需；Metaso 可选（提升额度） |
...
| `[verifier].verdict_policy` | 验证器裁决策略 | `hunt` | 当前唯一策略。将验证器的 `pass`/`partial`/`fail` 映射为 `hunted`/`wounded`/`escaped` 语义。未知值会被拒绝，为未来策略预留扩展 |
