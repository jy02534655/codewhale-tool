...
- 所有消息已本地化
- 内部使用 `ConfigEngine` 通用 `get`/`set` 方法读写本地 store，不含业务专用方法
...
2. **再看启动合并**：`initSync()` 函数（第 89-197 行）
3. **然后看实时同步**：`syncToCodeWhale()` 函数（第 214-267 行）
4. **最后看组合操作**：`_syncAfter` 和各组合方法（第 67 行，第 279-322 行）
